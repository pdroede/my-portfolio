      ...
      {labelsToShow.map((item) => {
        ...
        return (
          ...
          >
            <motion.span
              animate={{ opacity }}
              className="whitespace-nowrap text-xs text-chart-label"
              initial={{ opacity: 1 }}
...
export interface BarChartProps {
  data: Record<string, unknown>[]
  xDataKey?: string
  /** Chaves extra nos dados para calcular min/max do eixo Y (ex.: linhas sem Bar). */
  yDomainKeys?: string[]
...
function BarChartInner({
  ...
  const yScale = useMemo(() => {
    ...
    const pad = span > 0 ? span * 0.06 : Math.abs(maxValue) * 0.05
