        ...
        <article className="min-w-0 flex-1 max-w-[700px] pb-32">
          ...
          <section id="sp-framework" className="scroll-mt-20">
            ...
            <Reveal delay={0.07}>
              <ArticleProse className="mt-6 space-y-4">
                <p>
                  Seja <InlineMath math={tex`P_t`} /> o preço de fechamento do S&amp;P 500 e <InlineMath math={tex`r_t = \log(P_t / P_{t-1})`} /> o log-retorno diário. A <strong>volatilidade realizada</strong> anualizada sobre uma janela de <InlineMath math={tex`h`} /> dias é:
                </p>
              </ArticleProse>
            </Reveal>
            ...
            <Reveal delay={0.1}>
              <Definition
                ...
                formula={tex`\text{RV}_h = \sqrt{\frac{252}{h} \sum_{i=1}^{h} r_{t-i}^2}`}
            ...
            <Reveal delay={0.07}>
              <ArticleProse className="space-y-4">
                <p>
                  A variável-alvo é o logaritmo da volatilidade realizada <em>forward-looking</em> no horizonte <InlineMath math={tex`h`} />:
                </p>
              </ArticleProse>
            </Reveal>
            <Reveal delay={0.1}>
              <Definition
                id="3.2"
                name="Variável-alvo"
                formula={tex`\hat{y}_{t,h} = \log\!\left(\text{RV}_h^{\text{forward}}\right), \quad \text{RV}_h^{\text{forward}} = \sqrt{\frac{252}{h}\sum_{i=1}^{h} r_{t+i}^2}`}
            ...
            <Reveal delay={0.07}>
              <ArticleProse className="space-y-4">
                <p>
                  O <strong>Prêmio de Variância</strong> (<InlineMath math={tex`\text{VRP}`} />) captura a diferença entre variância implícita e realizada — sinal de quanto os formadores de opções cobram de prêmio de risco:
                </p>
              </ArticleProse>
            </Reveal>
            <Reveal delay={0.1}>
              <Definition
                ...
                formula={tex`\text{VRP}_t = \text{IV}_t^2 - \text{RV}_{22,t}^2`}
            ...
            <Reveal delay={0.07}>
              <ArticleProse className="space-y-4">
                <p>
                  A métrica de avaliação principal segue Goyal-Welch: o <strong><InlineMath math={tex`R^2`} /> fora de amostra</strong>, que compara o modelo contra o benchmark de prever a média histórica da série:
                </p>
              </ArticleProse>
            </Reveal>
            <Reveal delay={0.1}>
              <Definition
                ...
                formula={tex`R^2_{\text{OOS}} = 1 - \frac{\sum_{t}(y_t - \hat{y}_t)^2}{\sum_{t}(y_t - \bar{y})^2}`}
          ...
          <section id="sp-data" className="scroll-mt-20">
            ...
            <Reveal delay={0.08}>
              <ArticleProse className="mt-6 space-y-4">
                <p>
                  Os dados são obtidos via Yahoo Finance cobrindo o período de 2010 a 2024, com atualização diária. Os instrumentos utilizados são o próprio S&amp;P 500 (<InlineMath math={tex`\texttt{\^{}GSPC}`} />) e a família de índices VIX: VIX de 30 dias (<InlineMath math={tex`\texttt{\^{}VIX}`} />), VIX de 9 dias (<InlineMath math={tex`\texttt{\^{}VIX9D}`} />), VIX de 3 meses (<InlineMath math={tex`\texttt{\^{}VIX3M}`} />), VIX de 6 meses (<InlineMath math={tex`\texttt{\^{}VIX6M}`} />) e o índice SKEW (<InlineMath math={tex`\texttt{\^{}SKEW}`} />).
                </p>
                <p>
              ...
              {[
                {
                  ...
                  body: <>Nível absoluto da volatilidade implícita nos quatro vencimentos: <InlineMath math={tex`\text{VIX}_{9d},\, \text{VIX}_{30d},\, \text{VIX}_{3m},\, \text{VIX}_{6m}`} /> e sua versão em log. Captura o regime geral de volatilidade implícita do mercado.</>,
                },
                {
                  n: '4.2', title: 'Estrutura a Termo (4 features)',
                  body: <>Inclinação da curva de volatilidade implícita: diferenças entre vencimentos curtos, longos e spread total (<InlineMath math={tex`\text{VIX}_{3m} - \text{VIX}_{9d}`} />). <em>Contango</em> (curva normal, IV crescente com prazo) vs. <em>backwardation</em> (inversão de curva, sinal de estresse) são estados de regime com comportamento preditivo distinto.</>,
                },
                {
                ...
                },
                {
                  ...
                  body: <><InlineMath math={tex`\text{RV}_5`} /> (semanal) e <InlineMath math={tex`\text{RV}_{22}`} /> (mensal) em log-escala, e dois ratios <InlineMath math={tex`\text{RV}_5 / \text{RV}_{22}`} /> que capturam aceleração recente de volatilidade. Fundamento do modelo HAR (Corsi, 2009).</>,
                },
                {
                  n: '4.5', title: 'Prêmio de Variância (3 features)',
                  body: <><InlineMath math={tex`\text{VRP}_{30d}`} />, <InlineMath math={tex`\text{VRP}_{\text{short}}`} /> e um indicador direcional de sinal do VRP. Diferencial entre variância implícita e realizada; persistente e historicamente associado a predictabilidade de volatilidade.</>,
                },
                {
          ...
          <section id="sp-models" className="scroll-mt-20">
            ...
            <Reveal delay={0.08}>
              <ArticleProse className="mt-6 space-y-4">
                <p>
                  O pipeline emprega quatro preditores organizados da menor para a maior complexidade, mais um ensemble. A comparação interna revela o que cada camada de complexidade adiciona em termos de <InlineMath math={tex`R^2_{\text{OOS}}`} /> — e onde a lei dos retornos decrescentes entra.
                </p>
              </ArticleProse>
            </Reveal>
              ...
              {[
                {
                  ...
                  body: <>Usa o próprio VIX como previsão direta: <InlineMath math={tex`\hat{y}_t = \log(\text{VIX}_t / 100)`} />. Benchmark natural: qualquer modelo que não supere o VIX fora de amostra é descartado. Na literatura, superar o VIX é não-trivial — ele já incorpora expectativa coletiva de mercado.</>,
                },
                {
                  n: 'M.2', title: 'HAR-RV+IV',
                  body: <><em>Heterogeneous Autoregressive model</em> com volatilidade implícita, baseado em Corsi (2009). Regressão linear sobre 5 features selecionadas: <InlineMath math={tex`\log(\text{RV}_5),\, \log(\text{RV}_{22}),\, \log(\text{VIX}),\, \text{VRP}_{30d},\, \text{SKEW}`} />. Transparente e interpretável; captura persistência de volatilidade em múltiplas escalas de tempo.</>,
                },
                {
                  n: 'M.3', title: 'Lasso (seleção esparsa)',
                  body: <>Regressão <InlineMath math={tex`\ell_1`} />-penalizada sobre todas as 24 features. A penalidade zera coeficientes de features não-informativas, gerando um modelo esparso e interpretável. Captura efeitos lineares que o HAR não usa, sem sobreajustar por seleção manual de variáveis.</>,
                },
                {
                  n: 'M.4', title: 'XGBoost',
                  body: <>Gradient boosting com árvores rasas (<InlineMath math={tex`d_{\max}=4`} />) para captura de não-linearidades e interações entre features. Usado também para extração de valores SHAP — atribuição de importância por feature em nível de previsão individual. Não domina os modelos lineares uniformemente, mas complementa o ensemble.</>,
                },
                {
                  n: 'M.5', title: 'Ensemble (média simples)',
                  body: <>Média aritmética das previsões de HAR, Lasso e XGBoost. Reduz variância de modelo específico sem introduzir parâmetros adicionais. A diversidade entre modelos lineares e não-lineares gera ganho de <InlineMath math={tex`R^2_{\text{OOS}}`} /> consistente sobre qualquer modelo individual.</>,
                },
              ].map(({ n, title, body }, i) => (
                <Reveal key={n} delay={i * 0.05}>
                  <div className="border-zinc-800/40 bg-zinc-900/20 rounded-xl border p-5">
            ...
            <Reveal delay={0.1}>
              <Definition
                ...
                formula={tex`\mathcal{D}_k^{\text{train}} = \{(x_t, y_t)\}_{t=1}^{T_k}, \quad \mathcal{D}_k^{\text{test}} = \{(x_t, y_t)\}_{t=T_k+1}^{T_k + \Delta}`}
          ...
          <section id="sp-results" className="scroll-mt-20">
            ...
            <div className="mt-8 grid grid-cols-2 gap-3">
              <MetricCard symbol={tex`R^2_{\text{OOS}}`} label="Ensemble · h = 5d" value="≈ 0.40" delay={0.05} />
              <MetricCard symbol={tex`\text{MAE}`} label="Erro médio (p.p. vol)" value="≈ 2.8 pp" delay={0.10} />
              <MetricCard symbol={tex`h^*`} label="Horizonte ótimo" value="5–10 d" delay={0.15} />
              <MetricCard symbol={tex`|\mathcal{F}|`} label="Total de features" value="24" delay={0.20} />
            </div>

            <Reveal delay={0.1}>
              <ArticleProse className="mt-10 space-y-5">
                <p>
                  O ensemble alcança <InlineMath math={tex`R^2_{\text{OOS}} \approx 0.40`} />, superando o VIX Naive (baseline) de forma consistente. Na literatura de previsão de volatilidade, <InlineMath math={tex`R^2_{\text{OOS}} > 0.20`} /> já é considerado resultado robusto; valores próximos de 0.40 são expressivos considerando que o benchmark já incorpora expectativas de mercado.
                </p>
                <p>
                ...
                <p>
                  <strong>Interpretabilidade SHAP:</strong> os valores SHAP do XGBoost mostram que as features de maior importância agregada são <InlineMath math={tex`\log(\text{RV}_{22})`} />, <InlineMath math={tex`\text{VRP}_{30d}`} /> e a inclinação da estrutura a termo — resultado consistente com a seleção do Lasso, que converge nas mesmas variáveis como features de maior peso.
                </p>
              </ArticleProse>
            </Reveal>
          </section>
